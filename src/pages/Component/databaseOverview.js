          ...
          {appAcionLogUtil.isShowCommitInfo(data) ? (
            <div className={styles.codeVersion}>
              <div className={styles.versionInfo}>
                <FormattedMessage id="componentOverview.body.tab.LogItem.information" />{' '}
                {appAcionLogUtil.getCommitLog(data)}
              </div>
...
export default class Index extends PureComponent {
  constructor(arg) {
    ...
    this.state = {
      ...
      page: 1,
      page_size: 6,
      // KubeBlocks 组件资源配置 (从 clusterDetail.resource 获取)
      disk: 0, // 存储容量，单位 MB
      memory: 0, // 内存容量，单位 MB
      originalMemory: 0,
      originalStorage: 0,
      ...
      pages: 1,
      pageSize: 10,
      total: 0,  // 替代传统的 fetchAppDiskAndMemory，直接使用数据库组件的资源配置
      status: '',
      isopenLog: false,
      buildSource: null,
  ...
  componentWillReceiveProps(nextProps) {
    const {
      status: newStatus,
      timers: newTimers,
      appDetail: newAppDetail,
      clusterDetail: newClusterDetail
    } = nextProps;
    const { status, timers, appDetail, clusterDetail } = this.props;
    if (newStatus !== status) {
    ...
    // 当 clusterDetail 变化时，重新提取 KubeBlocks 资源数据
    if (newClusterDetail !== clusterDetail) {
      // 使用新的 clusterDetail 更新资源数据
      setTimeout(() => {
        this.extractKBResourceData();
      }, 100);
    }
  ...
  // 从 KubeBlocks clusterDetail.resource 中提取内存和存储数据
  extractKBResourceData = () => {
    const { clusterDetail } = this.props;
    const resource = clusterDetail && clusterDetail.resource;

    if (resource) {
      const memoryInMB = resource.memory || 0;
      const storageInGi = resource.storage || 0;

      this.setState({
        disk: storageInGi,
        memory: memoryInMB,
        originalMemory: resource.memory || 0,
        originalStorage: resource.storage || 0
      });
    } else {
      this.setState({
        disk: 0,
