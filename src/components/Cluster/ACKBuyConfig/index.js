import { Button, Col, Form, Input, InputNumber, Modal, Select, Typography } from 'antd';
...
export default class ACKBuyConfig extends PureComponent {
  ...
  createCluster = () => {
    const { form, dispatch, eid, selectProvider } = this.props;
  ...
  cancelShowCreateDetail = () => {
    ...
    const { showClusters } = this.props;
    showClusters()
  };

  render() {
    const { showClusters, eid } = this.props;
    ...
    return (
      <div>
        <Form>
          <Col span={24} style={{ padding: '16px' }}>
            <Paragraph className={styles.describe}>
              <ul>
                <li>
                  <span>委托购买时将购买以下资源：标准托管集群(1个)、VPC(1个)</span>
                </li>
                <li>
                  <span>新购集群都将采用按需后付费模式，因此请确保你的阿里云账号能够进行按需购买资源</span>
                </li>
                <li>
                  <span>
                ...
                <li>
                  <span>购买集群的成功性和耗时取决于阿里云的实际状态，正常情况下预计10分钟可以完成集群购买</span>
                </li>
                {customResource &&
                  <li>
                    <span>
            ...
            <Form.Item label="名称">
              {getFieldDecorator('name', {
                ...
                rules: [
                  ...
                  { pattern: /^[a-z0-9A-Z-]+$/, message: '只支持字母、数字和中划线组合' },
                ],
            ...
            {!customResource &&
              <Form.Item label="资源配置">
                {getFieldDecorator('resourceType', {
                  ...
                  <Select onChange={this.changeWorkerType} placeholder="集群名称">
              ...
              <Form.Item label="资源配置">
                {getFieldDecorator('resourceType', {
                  ...
                  rules: [
                    ...
                    { pattern: /^[a-z0-9A-Z.]+$/, message: '请确定资源配置规格是否合法' },
                  ],
          ...
          <Col style={{ textAlign: 'center', marginTop: '32px' }} span={24}>
            <Button onClick={showClusters} style={{ marginRight: '16px' }}>
              返回
            </Button>
            <Button
        ...
        {lastCreateTaskShow &&
          <ConfirmModal
            title="新的集群不能开始购买"
            desc="在当前提供商你上一次购买任务还没完成，是否进入查看详情"
            onCancel={this.onCancelShowLastTask}
            onOk={this.showCreateDetail}
          />}
        {showTaskDetail &&
          showTask &&
