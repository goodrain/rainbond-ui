    ...
    constructor(props) {
      ...
      this.state = {
        ...
        apps: this.props.batchDeleteApps.map((item) =>{
            return {service_id:item.service_id,service_cname:item.service_cname,msg:"正在删除",status:0}
        }),
        confirm: false
      };
    }
        ...
        return (
            <Modal
              ...
              {this.state.confirm ? (<Table
                ...
                <Row>
                {this.state.apps.map((item) => {
                  return <Col span={8} key={"col"+item.service_id} style={{overflow:"auto"}}><p key={item.service_id}>{item.service_cname}</p></Col> 
                })}
                </Row>
                <Button type="primary" onClick={this.handleDelete}>确实批量删除</Button></div>}
