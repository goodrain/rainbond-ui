        ...
        form.validateFields((err, fieldsValue) => {
            ...
            arr.map((item) => {
                if (item.id == editData.id) {
                    ...
                    item.envs.map((item) => {
                        item.name == "BUILD_MAVEN_CUSTOM_OPTS" ? item.value = fieldsValue.BUILD_MAVEN_CUSTOM_OPTS : ""
                        item.name == "BUILD_PROCFILE" ? item.value = fieldsValue.PROCFILE : ""
                    })
                }
            })
            this.setState({
    ...
    render() {
        const columns = [
            ...
            },
            {
                ...
                render: (val, index) => {
                    return <span key={val}>{index.ports && index.ports.length > 0 ? index.ports[0].container_port : val+ 5000}</span>
                }
            },
            ...
            <div>
                <div style={{
                    ...
                    <div
                        ...
                        <Alert message="以下为检测出的Maven多模块项目的模块信息, 请选择需要构建的模块, 并确认构建信息" type="success" />
