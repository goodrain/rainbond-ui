...
import { Card, Modal, Checkbox, notification, } from 'antd';
...
class EditorToplogy extends PureComponent {
  ...
  handleOk = (e) => {
    const { name, id, checkedValues } = this.state;
    addRelationedApp({
      team_name: globalUtil.getCurrTeamName(),
      app_alias: name,
      dep_service_id: id,
      open_inner: true,
      container_port: Number(checkedValues)
    }).then((res) => {
      if (res.code==200) {
  ...
  render() {
    ...
    return (
      <Card style={{ minHeight: 400 }} bordered={false}>
        ...
        <Modal
          title="要关联的服务暂未开启对内端口，是否打开"
          visible={this.state.visible}
          onOk={this.handleOk}
          onCancel={this.handleCancel}
        >
          <div>
            <CheckboxGroup options={list} onChange={(checkedValues) => { this.onChange(checkedValues) }} />
          </div>
        </Modal>
        <GGEditor>
