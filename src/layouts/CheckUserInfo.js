...
export default class CheckUserInfo extends React.PureComponent {
  ...
  toDefaultTeam = () => {
    ...
    // 当前团队里没有数据中心
    const currRegion = team.region[0] ? team.region[0].team_region_name : 'no-region';
    this.props.dispatch(routerRedux.replace(`/team/${team.team_name}/region/${currRegion}/index`));
  };
  // 判断当前用户有没有团队
  hasTeam = () => {
  ...
  canCreateTeam = () => {
    const currentUser = this.props.userInfo;
    return userUtil.isSystemAdmin(currentUser) || userUtil.isCompanyAdmin(currentUser);
  };
  // 验证当前团队里是否已经开通了数据中心
  currTeamHasRegion = () => {
    const user = this.props.userInfo;
  ...
  checkUrlTeamRegion = () => {
    ...
    if (!currTeam || !currRegion) {
      ...
      if (currTeam && currRegion) {
        this.props.dispatch(routerRedux.replace(`/team/${currTeam}/region/${currRegion}/index`));
      } else {
        this.toDefaultTeam();
      }
      return false;
    ...
    if (!team.region || !team.region.length) {
      this.props.dispatch(routerRedux.replace(`/team/${currTeam}/region/no-region/index`));
      return false;
    }

    // 判断当前浏览的数据中心是否在要访问的团队里
    const region = team.region.filter(region => region.team_region_name === currRegion);
    if (!region.length) {
      this.toDefaultTeam();
      return false;
    }
  ...
  render() {
    ...
    if (!this.hasTeam() && !this.canCreateTeam()) {
      return <WelcomeAndJoinTeam rainbondInfo={rainbondInfo} onOk={this.props.onInitTeamOk} />;
    }
