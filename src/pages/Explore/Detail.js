...
class ExploreDetail extends PureComponent {
  ...
  handleInstallSubmit = () => {
    ...
    const { detail, selectedVersion, selectedTeam, isDeploy } = this.state;
    ...
    form.validateFields((err, values) => {
      ...
      runMarketInstallPreflight({
        ...
        onCancel: () => this.setState({ submitLoading: false }),
        onError: (error) => {
          this.setState({ submitLoading: false });
          handleAPIError(error);
        }
            ...
            teamLoading || teamList.length === 0 ? null : (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  ...
                  <Radio
                    checked={isDeploy}
                    onClick={() => this.setState(prevState => ({ isDeploy: !prevState.isDeploy }))}
                  >
