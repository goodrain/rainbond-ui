//共同的信息
    ...
    const button = {
        ...
        'guideCarousel.appOverview.components.leftClick.item1':'组件总览：展示组件运行的关键信息。',
        'guideCarousel.appOverview.components.leftClick.item2':'资源分配：查看 CPU、内存、磁盘等资源是否够用。',
        'guideCarousel.appOverview.components.leftClick.item3':'日志与监控：排查报错、性能波动和运行异常。',
        'guideCarousel.appOverview.components.leftClick.item4':'环境、依赖和高级设置：调整变量、组件关系和更细的配置项。',
        'guideCarousel.appOverview.components.rightClick.title':'鼠标右键（打开菜单）',
        'guideCarousel.appOverview.components.rightClick.item1':'创建依赖：创建组件连接，让它们可以互相访问。',
        'guideCarousel.appOverview.components.rightClick.item2':'启动、关闭、删除组件：对组件进行操作，快速调整运行状态。',
        'guideCarousel.appOverview.components.rightClick.item3':'执行更新：让修改的配置、镜像或版本快速生效。',
        'guideCarousel.appOverview.components.rightClick.item4':'web终端：直接进入组件容器，做在线查看和临时处理。',
        ...
        'guideCarousel.appOverview.gateway.leftClick.title':'鼠标左键',
        'guideCarousel.appOverview.gateway.leftClick.item1':'应用网关路由：配置域名、路径和访问规则，让用户能正确访问应用。',
        'guideCarousel.appOverview.gateway.leftClick.item2':'应用网关证书：维护 HTTPS 证书，保障访问更安全。',
        'guideCarousel.appOverview.gateway.leftClick.item3':'应用网关目标服务：确认外部流量最终会转发到哪些组件。',
        'button.open':'开通',
        'button.submit':'提交',
