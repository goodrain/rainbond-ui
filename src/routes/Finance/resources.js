...
export default class BasicList extends PureComponent {
  ...
  getResPrice() {
    const regionName = this.props.match.params.regionName;
    moment(this.state.endDate || this.state.defaultEndDate).diff(
    ...
    this.props.dispatch({
      ...
      payload: {
        ...
        rent_time: moment(this.state.endDate || this.state.defaultEndDate).diff(
          moment(this.state.defaultStartDate),
          'days',
        ),
      },
  ...
  render() {
    ...
    memoryMasks[usedMemory] = (
      <Tooltip title={`当前使用量: ${sourceUnit.unit(usedMemory, 'MB')}`}>
        <Icon type="up" />
      </Tooltip>
    );
    memoryMasks[limitMemory] = (
      <Tooltip title={`已包月: ${sourceUnit.unit(limitMemory, 'MB')}`}>
        <Icon type="up" />
      </Tooltip>
    );
    ...
    const diskMasks = {};
    diskMasks[usedDisk] = (
      <Tooltip title={`当前使用量: ${sourceUnit.unit(usedDisk, 'GB')}`}>
        <Icon type="up" />
      </Tooltip>
    );
    diskMasks[limitDisk] = (
      <Tooltip title={`已包月: ${sourceUnit.unit(limitDisk, 'GB')}`}>
        <Icon type="up" />
      </Tooltip>
    );
    const money = this.state.money;

    const pageHeaderContent = null;
    return (
      <PageHeaderLayout
        breadcrumbList={[
          {
          ...
          <div>
            <div>
              ...
              <Row>
                ...
                <Col span={16}>
                  <Slider
                    tipFormatter={value => sourceUnit.unit(value, 'MB')}
                    value={this.state.buymemory || 1}
                    onChange={this.checkMemoryChange}
                    onAfterChange={this.handleMemoryChange}
                    step={512}
                    min={0}
                    max={1024 * 100}
                    marks={memoryMasks}
                ...
                <Col span={4} style={{ fontSize: '16px', paddingTop: 8 }}>
                  {' '}
                  {sourceUnit.unit(this.state.buymemory, 'MB')}{' '}
                </Col>
              </Row>
              <Row>
                <Col span={2} style={{ fontSize: '16px', paddingTop: 8 }}>
                ...
                <Col span={16}>
                  <Slider
                    ...
                    tipFormatter={value => sourceUnit.unit(value, 'GB')}
                    value={this.state.buydisk || 1}
                    onAfterChange={this.handleDiskChange}
                    step={1}
                    min={0}
                    max={1024}
                    marks={diskMasks}
                ...
                <Col span={4} style={{ fontSize: '16px', paddingTop: 8 }}>
                  {' '}
                  {sourceUnit.unit(this.state.buydisk, 'GB')}
                </Col>
              </Row>
                ...
                <Col span={22}>
                  <RangePicker
                    ...
                    disabledDate={current =>
                      current < moment(this.state.defaultEndDate, this.state.dateFormat)
                    }
                    value={[
                      moment(this.state.defaultStartDate, this.state.dateFormat),
                      moment(
                        this.state.endDate || this.state.defaultEndDate,
            ...
            <div>
              <h3>费用详情</h3>
              <p />
              <p>总费用： {money.total_fee || 0} 元</p>
              <p>已付费： {(money.total_fee - money.actual_fee || 0).toFixed(2)} 元</p>
              <p>应付费： {money.actual_fee || 0} 元</p>
            </div>
            <Row>
              <Col span="24">
                <Button onClick={this.buyPurchase} type="primary" style={{ marginRight: 8 }}>
                  确认购买
                </Button>
                <Button
                  onClick={() => {
