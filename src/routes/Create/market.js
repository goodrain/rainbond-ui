  ...
  handleCreate = (vals) => {
    ...
    this.props.dispatch({
      ...
      callback: () => {
        ...
        // 关闭弹框
        this.onCancelCreate();
        this.props.dispatch(routerRedux.push(`/team/${globalUtil.getCurrTeamName()}/region/${globalUtil.getCurrRegionName()}/groups/${
          vals.group_id
        }`));
      },
    });
  ...
  render() {
    ...
    const cardList = (
      <List
        ...
        dataSource={list}
        renderItem={item => (
          <List.Item style={{ border: 'none' }}>{this.renderApp(item)}</List.Item>
        )}
      />
    );
