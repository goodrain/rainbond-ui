...
export default {
  // 判断平台是否配置了github创建应用
  githubEnable: (bean = {}) =>
    (bean.github && bean.github.enable && bean.github.enable) || false,
  // 判断平台是否配置了gitlab(好雨git)创建应用
  gitlabEnable: (bean = {}) =>
    (bean.gitlab && bean.gitlab.enable && bean.gitlab.enable) || false,
  ...
  exportAppEnable: (bean = {}) =>
    (bean.export_app && bean.export_app.enable && bean.export_app.enable) ||
    false,
  // 判断平台是否配置了市场支持跨数据中心
  ...
  cloudMarketEnable: (bean = {}) =>
    (bean.cloud_market &&
      bean.cloud_market.enable &&
      bean.cloud_market.enable) ||
    false,
  // 判断平台是否配置了新手引导
  newbieGuideEnable: (bean = {}) =>
    (bean.newbie_guide &&
      bean.newbie_guide.enable &&
      bean.newbie_guide.enable) ||
    false,
  // 判断平台是否配置了官方Demo
  ...
  openDataCenterStatusEnable: (bean = {}) =>
    (bean.open_data_center_status &&
      bean.open_data_center_status.enable &&
      bean.open_data_center_status.enable) ||
    false,
  // 判断平台是否配置了具有文档权限
  documentEnable: (bean = {}) =>
    (bean.document && bean.document.enable && bean.document.enable) || false,
  // 判断平台是否配置了具有文档地址
